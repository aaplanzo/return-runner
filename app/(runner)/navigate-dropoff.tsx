...
export default function NavigateDropoff() {
  ...
  const [runnerCoords, setRunnerCoords] = useState(destination);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      locationSubRef.current = await Location.watchPositionAsync(
        { accuracy: Location.Accuracy.BestForNavigation, timeInterval: 4000, distanceInterval: 15 },
        (position) => {
          const coords = {
  ...
  function handleArrived() {
    ...
    router.replace('/(runner)/arrive-dropoff');
  }

  const eta = distM != null ? etaMinutes(distM) : null;
  const dropoffName = activeJob?.dropoff_name ?? activeJob?.retailer ?? 'UPS Store';
    ...
    <View style={styles.root}>
      ...
      >
        <Polyline
          ...
          strokeWidth={5}
        />
        <Marker coordinate={destination} anchor={{ x: 0.5, y: 1 }} title="Dropoff">
      ...
      {arrived && (
        ...
        >
          <Text style={styles.arrivedBannerText}>📍 You've arrived at the store!</Text>
        </Animated.View>
      )}
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + Spacing.md }]}>
        <View style={styles.bottomInfo}>
          <View style={styles.bottomLabelRow}>
            <Text style={styles.bottomLabel}>DROPOFF</Text>
            <View style={styles.pkgsChip}>
              <Text style={styles.pkgsChipText}>
                {activeJob?.package_count ?? 1} pkg
                {(activeJob?.package_count ?? 1) !== 1 ? 's' : ''}
              </Text>
            </View>
          </View>
          <Text style={styles.bottomName} numberOfLines={1}>{dropoffName}</Text>
          {eta != null && (
        ...
        >
          <Text style={[styles.arrivedBtnText, arrived && styles.arrivedBtnTextActive]}>
            {arrived ? "I've Arrived ✓" : "I've Arrived at Dropoff"}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  turnDistance: {
    fontFamily: FontFamily.bold,
    fontSize: 20, color: '#fff', lineHeight: 24,
  },
  turnStreet: {
    fontFamily: FontFamily.regular,
    fontSize: 13, color: 'rgba(255,255,255,0.75)', marginTop: 1,
  },
  etaBox: { alignItems: 'center' },
  etaValue: {
    fontFamily: FontFamily.bold,
    fontSize: 22, color: '#fff', lineHeight: 26,
  },
  etaUnit: {
    fontFamily: FontFamily.mono,
    fontSize: 10, color: 'rgba(255,255,255,0.6)', letterSpacing: 0.3,
  },
  ...
  bottomLabel: {
    fontFamily: FontFamily.mono,
    fontSize: 11, letterSpacing: 0.5, color: Colors.textSecondary,
  },
  pkgsChip: {
    backgroundColor: '#EDF7F1',
    borderRadius: 6,
  ...
  pkgsChipText: {
    fontFamily: FontFamily.mono,
    fontSize: 10, letterSpacing: 0.3, color: Colors.success,
  },
  bottomName: {
    fontFamily: FontFamily.bold, fontSize: 16, color: Colors.textPrimary,
  },
  bottomEta: {
    fontFamily: FontFamily.regular, fontSize: 13, color: Colors.textSecondary,
  },
  arrivedBtn: {
    borderWidth: 2,
  ...
  arrivedBtnText: {
    fontFamily: FontFamily.bold, fontSize: 16, color: Colors.success, letterSpacing: 0.2,
  },
  arrivedBtnTextActive: { color: '#fff' },
});
